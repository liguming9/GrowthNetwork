...
The `web` directory contains two deliberately separate parts of the research prototype. The opening **Living Trace / Pre-exhibition study** simulates a gallery visit so that test participants can generate behavioural input. The later **Inside Us** interface is the proposed post-visit exhibition website. It uses the same HTML Canvas 2D geometry rather than a screenshot. The page reads `web/data/web_network.json`, which preserves all four node positions, arterial centrelines, recursive capillary segments, parent identifiers, hierarchy levels, taper widths, and deterministic timing from the current Blender JSON. The supplied Brain, Eye, Heart, and Lung PNGs remain visible from the first frame, while deterministic blue parent-child vessels grow inside their transparent silhouettes.

The study begins with **Enter** and lets a participant open Brain, Eye, Heart, and Lung in any order. Each detail view measures real viewing time; completing the fourth exhibit records the anonymous visitor ID, first-view sequence, per-exhibit dwell totals, transitions, and individual view events. A dedicated **Visit Complete** screen then marks the boundary between data generation and the post-visit website. Selecting **View My Journey** opens Inside Us in the **My Journey** mode and displays the participant's actual recorded order, total time, longest stop, and anonymous ID. Its purple route reuses the exact existing arterial points and node-rooted capillary branches from `web_network.json`: no new path geometry is generated. Pointer movement replays the chosen order, while longer viewing selects more of the existing local branch hierarchy.

Inside Us also provides a distinct **Shared Space** mode. It prioritises the red session trace, a small set of source-derived visitor-facing totals, and the blue accumulated Exhibition Memory. The mode switch changes the surrounding information hierarchy; the three layer controls inside the visualisation remain independent, so visitors can inspect On-site data, Exhibition Memory, or Your Journey without conflating their meanings. Editorial sections below the live network introduce the exhibition, the three scales of memory, the movement-to-trace process, the visitor-facing method, proposed exhibition information, and research context.
...
The browser uses three independent clocks so session-level participation, longer exhibition memory, and one participant's journey are not conflated. The segmented control above the network selects which clock the pointer scrubs. **On-site data** controls the red connections between exhibits; **Exhibition memory** controls only the blue vessels on the anatomical images; **Your journey** controls the purple overlay made from the recorded study visit. On-site arteries replay all visitor movements at their deterministic, staggered start times. A traversal begins at its real source node, so reverse records grow from the opposite end of the same centreline. Every completed traversal reinforces that centreline's local thickness; no parallel copy is created. On desktop, left-to-right means beginning-to-present. The network is proportionally reduced and shifted into the right-hand field so it does not collide with the editorial title. On mobile, the complete network rotates clockwise by 90 degrees and fills a taller stage; organ images and labels are counter-rotated so they remain upright. An upward swipe advances the selected timeline by relative gesture distance, while a downward swipe rewinds it. The animation stage retains the gesture until the selected layer reaches 100%; a new upward swipe is then handed back to native page scrolling so the visitor can continue down the website. Switching modes, replaying, or scrubbing one clock never changes the others.

A responsive editorial calendar covers 1–30 July 2026 and marks the prototype visit date of 10 July. Exhibition-memory movement updates the axis in real time: the active day grows typographically, turns blue, and automatically scrolls into the centre while the On-site data clock remains unchanged. Every calendar day is a button that shows the represented blue anatomical-memory accumulation for that date. The previous technical controls remain available internally for debugging but are not shown in the normal visitor-facing interface. Canvas rendering uses real high-DPI pixels capped at devicePixelRatio 3.
